...
const validatorSchema = {
  ...
  register: Yup.object().shape({
    ...
    confirmPassword: Yup.string()
      .oneOf([Yup.ref("password")], getErrorMessage("auth.errors.passwordsDoNotMatch"))
      .required(getErrorMessage("auth.errors.requiredField")),
  }),
  ...
  verifyOTP: Yup.object().shape({
    code: Yup.string()
      ...
      .matches(AUTH_CODE_VERIFICATION, getErrorMessage("auth.errors.invalidCode"))
      .required(getErrorMessage("auth.errors.requiredField")),
  }),
  ...
  changePassword: Yup.object().shape({
    ...
    newPassword: Yup.string()
      .required(getErrorMessage("auth.errors.requiredField"))
      .min(6, getErrorMessage("auth.errors.passwordTooShort"))
      .notOneOf([Yup.ref("oldPassword")], getErrorMessage("auth.errors.passwordsDoNotMatch")),
    confirmPassword: Yup.string()
      .required(getErrorMessage("auth.errors.requiredField"))
      .oneOf([Yup.ref("newPassword")], getErrorMessage("auth.errors.passwordsDoNotMatch")),
  }),
};
